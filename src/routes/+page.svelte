...
<main>
	...
	<div class="controls">
		<button onclick={handleConnect} disabled={!isSupported || isConnecting || isConnected}>
			{#if isConnected}
				接続済み
		...
		<button onclick={addSlot}>＋ スロット追加</button>
		<button onclick={startPlay} disabled={!isConnected}>▶ 再生</button>
		<button onclick={stopPlay}>■ 停止</button>
		<button onclick={sendOnce} disabled={!isConnected}>今のスロットを単発送信</button>
		<button
			onclick={() => {
				slots = tmp[0];
			}}>テンプレート 1</button
			...
			<section class="slot">
				...
				<label>
					...
					<textarea
						...
						oninput={(e) => updateSlot(idx, 'text', (e.currentTarget as HTMLTextAreaElement).value)}
					></textarea>
				</label>
				<label>
					ディレイ (ms)
					<input
						...
						oninput={(e) => updateSlot(idx, 'delayMs', (e.currentTarget as HTMLInputElement).value)}
					/>
				</label>
				<div class="slot-buttons">
					<button type="button" onclick={() => setCurrent(idx)}>ここから再生</button>
					<button type="button" onclick={() => removeSlot(idx)}>削除</button>
				</div>
			</section>
		{/each}
	</div>
